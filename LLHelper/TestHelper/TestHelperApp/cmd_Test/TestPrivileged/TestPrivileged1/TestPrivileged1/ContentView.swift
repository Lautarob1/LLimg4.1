...
//  TestPrivileged1
//
//  Created by EFI-Admin on 12/20/23.
//
...
func execCmdwAdminRightsOSA(command: String) {
...
func runCommand()  {
        let process = Process()
        print("inside runCommand")
        process.launchPath = "/usr/sbin/asr"
        process.arguments = ["restore", "--source", "/", "--target", "/Volumes/test1", "--erase", "--noprompt"]
    ...
    var body: some View {
        VStack {
            ...
            Button("Run ASR Command") {
                ...
                output=executeSudoCommand(command: "asr restore --source / --target /Volumes/test4 --erase --noprompt", passw: "Wait4Jesus99")
            }.padding()
            
            Button("Test SUDO") {
                output=executeSudoCommand(command: "sudo -S whoami", passw: "Wait4Jesus99")}.padding()
            
            HStack {
                Button("hdiutil Sparse Container") {
                    output=executeSudoCommand(command: "hdiutil create -size 500g -type SPARSE -fs APFS -volname test1 /Volumes/llidata/test1", passw: "Wait4Jesus99")
                }
                
                Button("Apple script test") {
                ...
                //            let mycommand="asr restore --source / --target /Volumes/test1 --erase --noprompt"
