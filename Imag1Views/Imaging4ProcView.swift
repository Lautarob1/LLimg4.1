...
//        let logfilePathEx = DiskDataManager.shared.selectedStorageOption + "/\(CaseInfoData.shared.imageName).LLeX"
        ...
        print2Log(filePath: logfilePathEx, text2p: sviewModel.output)
        if  sviewModel.output.contains("failed") {
            sviewModel.output="Container was not mounted"
            return
        }
...
//        let destinPath = DiskDataManager.shared.selectedStorageOption
        ...
        copyFF2sparse (fileItems: FileSelectionManager.shared.selectedFiles, destPath: destinPath)
//        timer.stopTimer()
...
//        let logfilePath = DiskDataManager.shared.selectedStorageOption + "/\(CaseInfoData.shared.imageName).info"
    ...
    func createSparseContainer () -> String {
        ...
        print("in createSparseCont (2): passw= \(passw)")
        let imgSize = "500g" // CaseInfoData.shared.imageName
    ...
    func copyFF2sparse (fileItems: [FileItem], destPath: String) {
